...
export const Gradient: React.FC<EnhancedGradientProps> = ({ className }) => {
  ...
  useEffect(() => {
    ...
    const drawScene = () => {
      ...
      const colors = [
        `hsl(43, ${30 + Math.sin(time) * 20}%, ${
          85 + Math.sin(time * 0.7) * 10
        }%)`,
        `hsl(30, ${40 + Math.cos(time * 1.1) * 20}%, ${
          75 + Math.sin(time * 0.8) * 10
        }%)`,
        `hsl(60, ${35 + Math.sin(time * 1.2) * 20}%, ${
          80 + Math.cos(time * 0.9) * 10
        }%)`,
        `hsl(20, ${45 + Math.cos(time * 0.9) * 20}%, ${
          70 + Math.sin(time * 1.1) * 10
        }%)`,
      ];
      ...
      gradients.forEach((gradient, index) => {
        ctx.globalAlpha =
          0.5 + 0.5 * Math.sin(time + (index * Math.PI) / gradients.length);
